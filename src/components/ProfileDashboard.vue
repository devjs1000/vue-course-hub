
<script setup>
import CourseCard from './CourseCard.vue'
import VideoPlayer from './VideoPlayer.vue'
</script>

<template>
  <div class="grid grid-cols-4 grid-rows-4 gap-4 p-4 h-screen bg-blue-500 w-full">
<div class="box row-span-2 col-span-1" >
  <img class='object-cover h-full' src="data:image/png;base64," alt="">
</div>
<div class="box row-span-2 col-span-3 overflow-hidden">
  <div class="flex overflow-scroll">
      <span v-for="course in courses" class="scale-[.7] h-full " :key="course.name">
        <CourseCard
          :name="course.name"
          :about="course.about"
          :tags="course.tags"
          :price="course.price"
          :category="course.category"
          :image="course.image"
        />
      </span>
    </div>
</div>
<div class="box row-span-2 col-span-1 px-2">
  <input type="text" class="p-input " placeholder="name" value="user name">
    <input type="email" class="p-input " placeholder="email" value="email@emal.com">
<textarea  cols="30" rows="6" class="p-input" >I am an explorer</textarea>
</div>
<div class="box row-span-2 col-span-1">

  <video-player title="recently watched" />
</div>
<div class="box row-span-2 col-span-2"></div>

  </div>



</template>

<script>

export default {
 data(){
   return {
     courses: [
        {
          name: "html",
          about: "best course ever",
          image:
            "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTyq-mRleja_C1p2NNnNW6DynWnaZwJS53UVg&usqp=CAU",
          price: 200,
          tags: ["web", "html", "basics"],
          category: "web development",
        },
        {
          name: "html",
          about: "best course ever",
          image:
            "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTyq-mRleja_C1p2NNnNW6DynWnaZwJS53UVg&usqp=CAU",
          price: 200,
          tags: ["web", "html", "basics"],
          category: "web development",
        },
        {
          name: "html",
          about: "best course ever",
          image:
            "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTyq-mRleja_C1p2NNnNW6DynWnaZwJS53UVg&usqp=CAU",
          price: 200,
          tags: ["web", "html", "basics"],
          category: "web development",
        },
        {
          name: "html",
          about: "best course ever",
          image:
            "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTyq-mRleja_C1p2NNnNW6DynWnaZwJS53UVg&usqp=CAU",
          price: 200,
          tags: ["web", "html", "basics"],
          category: "web development",
        },
        {
          name: "html",
          about: "best course ever",
          image:
            "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTyq-mRleja_C1p2NNnNW6DynWnaZwJS53UVg&usqp=CAU",
          price: 200,
          tags: ["web", "html", "basics"],
          category: "web development",
        },
      ],
   }
 }
}
</script>

<style>
.box{
  @apply bg-white rounded ;
}
</style>